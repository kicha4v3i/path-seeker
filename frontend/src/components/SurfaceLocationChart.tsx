...
export function SurfaceLocationChart({
  ...
  const cancelCameraAnimationRef = useRef<(() => void) | null>(null)
  // Remembers the camera angle the user last set (by rotating or picking a
  // preset view) so it survives data reloads, unit changes, and KOP edits.
  // Starts at the default view so an explicit camera is ALWAYS set and the
  // chart never auto-reframes when the axis ranges grow.
  const userCameraRef = useRef<SceneCamera>(CHART_VIEW_CAMERAS.default)
  const plotReadyRef = useRef(false)
  const viewAlignmentRef = useRef(viewAlignment)
  const pathStationsRef = useRef<SurveyStation[]>([])
  const lockedSceneAspectRef = useRef<ReturnType<typeof chartSceneAspect> | null>(null)
  ...
  function handlePlotRelayout(event?: Readonly<Record<string, unknown>>) {
    // Only remember the camera when the view itself changed (user drag/rotate,
    // zoom, or a preset animation). Ignore relayouts fired by axis/data
    // rebuilds — capturing those would overwrite the saved angle with a
    // default and let KOP/unit changes appear to reset the view.
    const cameraChanged =
      !!event &&
